...
namespace IFix
{
    ...
    {
        public Dictionary<int, List<Core.Instruction>> codes = new Dictionary<int, List<Core.Instruction>>();
        public HashSet<int> codeMustWriteToPatch = new HashSet<int>();
        public Dictionary<MethodReference, int> methodToId = new Dictionary<MethodReference, int>();
        public Dictionary<int, Core.ExceptionHandler[]> methodIdToExceptionHandler =
            new Dictionary<int, Core.ExceptionHandler[]>();

        public List<TypeReference> externTypes = new List<TypeReference>();
        public List<TypeReference> contextTypeOfExternType = new List<TypeReference>();
        public Dictionary<TypeReference, int> externTypeToId = new Dictionary<TypeReference, int>();
        public Dictionary<string, TypeReference> nameToExternType = new Dictionary<string, TypeReference>();
        public List<MethodReference> externMethods = new List<MethodReference>();
        public Dictionary<MethodReference, int> externMethodToId = new Dictionary<MethodReference, int>();

        public List<string> internStrings = new List<string>();
        public Dictionary<string, int> internStringsToId = new Dictionary<string, int>();

        public List<FieldReference> fields = new List<FieldReference>();
        public List<FieldDefinition> fieldsStoreInVirtualMachine = new List<FieldDefinition>();
        public Dictionary<FieldReference, int> fieldToId = new Dictionary<FieldReference, int>();

        public Dictionary<MethodReference, int> virtualMethodToIndex = new Dictionary<MethodReference, int>();
        const string Wrap_Perfix = "__Gen_Wrap_";

        int nextAllocId = 0;

        public bool isCompilerGenerated(TypeReference type)
        {
        ...
        public bool isCompilerGenerated(MethodReference method)
        {
        ...
        public bool isCompilerGenerated(FieldReference field)
        {
        ...
        public Dictionary<TypeDefinition, int> typeToCctor = new Dictionary<TypeDefinition, int>();
        public Dictionary<FieldDefinition, int> newFieldToCtor = new Dictionary<FieldDefinition, int>();
        ...
        //再补丁新增一个对原生方法的引用
        public int addExternType(TypeReference type, TypeReference contextType = null)
        {
        ...
        public bool isNewMethod(MethodDefinition method)
        {
            return configure.IsNewMethod(method);
        }

        public bool isNewClass(TypeDefinition type)
        {
            return configure.IsNewClass(type);
        }

        public bool isNewField(FieldDefinition field)
        {
            ...
            bool directCallVirtual = false, InjectType callerInjectType = InjectType.Switch)
        {
            ...
            if (method == null || (method.IsConstructor && !(isCompilerGeneratedPlainObject(method.DeclaringType) || isCustomClassPlainObject(method.DeclaringType)))
                ...
                && !isCompilerGenerated(method) && !(mode == ProcessMode.Patch && isNewMethod(method)) && mode != ProcessMode.Exec))
            {
        ...
        public TypeDefinition itfBridgeType;
        ...
        public TypeReference WrappersManagerType;
        public TypeDefinition wrapperMgrImpl;
        public MethodDefinition ctorOfWrapperMgrImpl;
        ...
        public List<MethodDefinition> anonymousTypeInfos = new List<MethodDefinition>();
        public Dictionary<MethodDefinition, int> anonymousTypeToId = new Dictionary<MethodDefinition, int>();
        ...
        public Dictionary<MethodReference, int> interfaceSlot = new Dictionary<MethodReference, int>();
        public Dictionary<MethodDefinition, MethodReference> implementMap
            = new Dictionary<MethodDefinition, MethodReference>();

        public List<TypeReference> bridgeInterfaces = new List<TypeReference>();
        ...
        void initEx(AssemblyDefinition assembly, AssemblyDefinition ilfixAassembly)
        {
            ...
            // addExternType(itfBridgeType);            // TODO(fishezhang)
        ...
        public bool ProcessEx(AssemblyDefinition assembly, AssemblyDefinition assembly_core)
        {
            this.mode = ProcessMode.Exec;
            this.configure = GenerateConfigure.Empty();

            initEx(assembly, assembly_core);
            emitWrapperManager();
            ...
            {
                var info = getMethodId(kv.Key, null, true);
                ++i;
            }
